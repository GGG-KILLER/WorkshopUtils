...
namespace WorkshopUtils.UI.GLuaHighlighter
{
	public partial class GLuaHighlightTokenizer
	{
		...
		private Boolean CanMove ( Int32 Delta )
		{
			...
			return -1 < newPos && newPos < this.Code.Length;
		}
		...
		private Char Read ( Int32 len = 1 )
		{
			if ( !CanMove ( len ) )
				return '\0';
			return this.Code[this.Position += len];
		}
		...
		{
			for ( Int32 i = this.Position ; i < this.Code.Length ; i++ )
			{
				if ( this.Code[i] == ch )
		...
		private Int32 IndexOf ( Func<Char, Boolean> Filter )
		{
			for ( Int32 i = this.Position ; i < this.Code.Length ; i++ )
			{
				if ( Filter?.Invoke ( this.Code[i] ) ?? default ( Boolean ) )
					return i - this.Position;
			}
		...
		public HighlightToken ReadComment ( )
		{
			...
			) )
			{
				...
				HighlightToken ret = ReadLongString ( init );
